...
export class ProfessorNotificationMigration implements Migration {
  ...
  run(): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      this.client.query(SQL, (err, res) => {
        if (err) {
          reject(
            `Error while applying ProfessorNotification migration; Stack: ${err}`
          );
        } else {
          resolve("");
        }
  ...
  drop(): Promise<string> {
    const SQL = `
            DROP TABLE IF EXISTS professor_notification;
    ...
    return new Promise((resolve, reject) => {
      this.client.query(SQL, (err, res) => {
        if (err) {
          reject(
            `Error while dropping ProfessorNotification table; Stack: ${err}`
          );
        } else {
          resolve("");
        }
