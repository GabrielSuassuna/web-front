...
function AuthRedirect(props) {
  ...
  if(props.levels.indexOf(AUTH_LEVELS.HEAD) !== -1 && userType === AUTH_LEVELS.PROFESSOR){
    apiRequest(
      "GET",
      ...
      (res) => {
        if(res.data.length === 0){
          alert("Você não tem permissão para ver essa página. Contate a chefia de departamento ou a coordenação do curso.")
          navigate('/home');
        }
      },
      (res) => {
